...
public class PHasher {
	...
	public String hash(byte[] salt){
		...
		//The value to be hashed is the concatenation of the salt and the password
	...
	//A version that does not change state, but allows specification of password and salt
	...
	public boolean match(String dbHash){
		...
		//Hash the stored password with the salt to see if it matches the stored hash
		String newHash = hash(salt);
		return dbHash.equals(newHash);
		
	}
	
	//Allow the user to match a hash with another password without changing state
